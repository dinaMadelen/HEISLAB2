...
func HandleConnection(conn *kcp.UDPSession, SendData chan<- string) {

}

func SendToMaster(receiver chan<- string, EL elevator) {
	conn, err := kcp.DialWithOptions("10.22.112.64:4000", nil, 10, 3)
	if err != nil {
	...
	for {
		...
		var package2 uint8 = uint8(EL.request[3][2]&0b1 | EL.request[3][0]&0b1<<1 | EL.request[2][2]&0b1<<2 | EL.request[2][1]&0b1<<3 | EL.request[2][0]&0b1<<4 | EL.request[1][2]&0b1<<5 | EL.request[1][1]&0b1<<6 | EL.request[1][0]&0b1<<3)
		...
		fmt.Println("Received from Master:", string(buffer[:n]))
