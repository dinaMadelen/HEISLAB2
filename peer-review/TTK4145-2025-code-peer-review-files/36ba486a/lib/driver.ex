# https://raw.githubusercontent.com/jostlowe/ttk4145_elixir_template/refs/heads/master/lib/driver.ex
defmodule Driver do
  use GenServer
  require Logger
  @call_timeout 1000
  ...
  def start_link([]) do
    start_link(address: {127, 0, 0, 1}, port: 15657)
  end

  def start_link(address: address, port: port) do
  ...
  def init([address, port]) do
    ...
    Logger.info("Driver active at #{inspect(address)}, port: #{port}")
    {:ok, socket}
  end
