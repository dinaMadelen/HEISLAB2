...
class Resource(T) {
    ...
    T allocate(int id, int priority){
        //Lock mutex
        mtx.lock();
        //Insert into priority queue, PriorityQueue automatically sorts
        queue.insert(id, priority);

        //While im not first
        //D does not like queue[0]
    ...
    void deallocate(T v){
        ...
        //Lock mutex
        mtx.lock();
        //Remove the one that just left the queue
        //D does not like queue=queue.remove[0]
        queue.popFront;
        value = v;
        // notify all and release the mutex
        cond.notifyAll();
        mtx.unlock();
