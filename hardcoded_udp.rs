...
fn main() -> std::io::Result<()> {
    ...
    let inn_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(10, 100, 23, 255)), 3700 + system_state.me_id as u16);
    let out_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(10, 100, 23, 255)), 3800 + system_state.me_id as u16);
    ...
    //----------------------------
    // replaced by hardcode start
    //----------------------------
    // for port in 3701..3799{
        // let inn_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(10, 100, 23, 255)),port as u16);
        // udphandler.send(&inn_addr, &new_online_msg);
    // }
    //--------------------------
    // replaced by hardcode end
    //--------------------------

    //----------------
    // hardcoded start
    //----------------
    let base_port: u16 = 3700;
    let port1: u16 = base_port + 1;
    let port2: u16 = base_port + 2;
    let port3: u16 = base_port + 3;

    let inn_addr1 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(10, 100, 23, 14)),port1 as u16);
    let inn_addr2 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(10, 100, 23, 17)),port2 as u16);
    let inn_addr3 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(10, 100, 23, 18)),port3 as u16);

    udphandler.send(&inn_addr1, &new_online_msg);
    udphandler.send(&inn_addr2, &new_online_msg);
    udphandler.send(&inn_addr3, &new_online_msg);
    //--------------
    // hardcoded end
    //--------------
