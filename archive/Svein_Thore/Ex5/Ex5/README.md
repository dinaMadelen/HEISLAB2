...
- Condition variables, Java monitors, and Ada protected objects are quite similar in what they do (temporarily yield execution so some other task can unblock us).
  - But in what ways do these mechanisms differ?
    ...
    for Condition variables you can lock, unlock, and release them untill a condition is met.
    for Java monitors will wait untill they are notified
    for Ada uses guards that block untill a certain condition is met.



- Bugs in this kind of low-level synchronization can be hard to spot.
  - Which solutions are you most confident are correct?
  Ada, its easier to just check the guard condition than the entire logic of the lifetime of a Java monitors or Condition variable.

  - Why, and what does this say about code quality?
    Because you dont have to check the entire function you can just check the guard conditions in ada entry guards. this makes the conditions for running the entry much more legible compared to java monitors or condition variables.
...
- We operated only with two priority levels here, but it makes sense for this "kind" of priority resource to support more priorities.
  - How would you extend these solutions to N priorities? Is it even possible to do this elegantly? 
    for some it is easy and intuitive to expand on others are less. forexample for the semaphores where you basically just have to add "else ifs" or the condition variables where it is already done, for the message passing it would be less elegant.
  - What (if anything) does that say about code quality?
    for this example i find the condition variable to be the best when it comes to expanding to more priority levels as it is already implemented. and i find the message passing with priority select to be the worst as it would require nested select blocks.
...
- In D's standard library, `getValue` for semaphores is not even exposed (probably because it is not portable – Windows semaphores don't have `getValue`, though you could hack it together with `ReleaseSemaphore()` and `WaitForSingleObject()`).
  - A leading question: Is using `getValue` ever appropriate?
    no, it is a snapshot of a value that can change imeadiatly after being read, so you cant trust the value you get from 'getValue'. it is "volataile" as it can unexpectadly change.
  - Explain your intuition: What is it that makes `getValue` so dubious? as mentioned above you cant trust the value you get from it as it can instantly change after being read.
...
- Which one(s) of these different mechanisms do you prefer, both for this specific task and in general? (This is a matter of taste – there are no "right" answers here)
  I think Ada is the most elegant one, but i also really like the condition variables. i dispise message passing as i find it less intuitive than the others and it is easier to risk somthing taking the same resource twice, in a mutex or ada based system it would lock instead. it wont automatically lock in message passing.
