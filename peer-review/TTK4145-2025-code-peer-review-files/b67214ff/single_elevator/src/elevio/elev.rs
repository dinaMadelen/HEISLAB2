...
impl ElevatorDriver {
    pub fn init(addr: &str, num_floors: u8) -> Result<ElevatorDriver> {
        Ok(Self {
            socket: Arc::new(Mutex::new(TcpStream::connect(addr)?)),
            num_floors,
        })
    }
    /// Start driving motor in direction
    ...
    ///  Not sure what this does, no return- value
    ...
    /// Checks status of obstruction switch
...
impl fmt::Display for ElevatorDriver {
    /// Writes  out scoket address and number of floors for an elevator
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let addr = self.socket.lock().unwrap().peer_addr().unwrap();
        write!(f, "ElevatorDriver@{}({})", addr, self.num_floors)
    }
}
