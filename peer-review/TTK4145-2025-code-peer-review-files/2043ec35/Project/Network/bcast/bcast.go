...
// it on `port`
func Transmitter(port int, stop_chan chan bool, enable_chan chan bool, chans ...interface{}) {
	checkArgs(chans...)
	...
	for {
		select {
		case <-stop_chan:
			fmt.Println("gått inn i kill seg selv")
			return
		case <-enable_chan:
			fmt.Println("waiting for tx")
			chosen, value, _ := reflect.Select(selectCases)
			fmt.Println("after select")
			jsonstr, marshalErr := json.Marshal(value.Interface())
			if marshalErr != nil {
				fmt.Println("error mashalling: ", marshalErr)
				continue
			}
...
func Receiver(port int, stop_chan chan bool, chans ...interface{}) {
	checkArgs(chans...)
	chansMap := make(map[string]interface{})
	for _, ch := range chans {
		//lager map med channels basert på hva de tar inn (chan int blir int)
		chansMap[reflect.TypeOf(ch).Elem().String()] = ch
	}

	var buf [bufSize]byte
	conn := conn.DialBroadcastUDP(port)
	//kan eventuelt lage ny kanal og konstant lese fra i hver loop og så sende på channel?
	for {
		select {
		case <-stop_chan:
			fmt.Println("dreper receiver")
			fmt.Println("2 gått inn i kill seg selv")
			return
		default:
