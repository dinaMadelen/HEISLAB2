...
print_help() {
  ...
  echo "Example, Elixir usage: sudo $0 50 -ie"
  echo "Options:"
  echo "  -i, --input    Apply packet loss to incoming traffic only"
  echo "  -o, --output   Apply packet loss to outgoing traffic only. Note that this may "
  echo "                 cause send() operations to return EPERM on dropped packet."
  echo "  -e, --elixir   Apply packet loss to detected elixir processes alongside the given ports."
  echo "  -u, --unsafe   Run the script without a 15-minute safety timeout, don't use if you ssh"
